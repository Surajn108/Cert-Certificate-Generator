...
function livePreview() {
  const name =
    document.getElementById("inp-name").value.trim() || "Recipient Name";
  const role =
    document.getElementById("inp-role").value.trim() || "Role / Title";
  const event =
    document.getElementById("inp-event").value.trim() ||
    "Event / Internship Name";
  const org =
    document.getElementById("inp-org").value.trim() || "Issuing Organisation";
...
async function downloadPDF() {
  if (!validate()) {
    showMsg("Fill in all required fields first.", "error");
    return;
  }
  livePreview();
  ...
  const name =
    document.getElementById("inp-name").value.trim() || "certificate";
...
function verifyCert() {
  ...
  if (!input) {
    resultEl.innerHTML =
      '<p style="color:var(--red)">Please enter a Certificate ID.</p>';
  ...
  const dateStr = rec.date
    ...
    : "—";
  const issuedStr = new Date(rec.issuedAt).toLocaleDateString("en-IN", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
